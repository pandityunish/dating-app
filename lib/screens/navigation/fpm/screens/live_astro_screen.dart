...
class LiveAstroScreen extends StatefulWidget {
  final String id;
  const LiveAstroScreen({super.key, required this.id});

  @override
  State<LiveAstroScreen> createState() => _FreeChatScreenState();
}

class _FreeChatScreenState extends State<LiveAstroScreen> {
  List<AdsModel> matchads = [];
  List<AdsModel> astroads = [];
  final ScrollController _scrollController = ScrollController();

  void getads() async {
    matchads = await AdsService().getallusers(adsid: "47");
    astroads = await AdsService().getallusers(adsid: "48");
